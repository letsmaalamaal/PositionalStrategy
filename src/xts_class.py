...
class XTS_parse:
    def __init__(self, API_KEY, API_SECRET):
        ...
        self.xt = XTSConnect(API_KEY, API_SECRET, self.source)
        self.response = self.xt.marketdata_login()
        
        print("Login:",self.response)
        self.response = self.xt.get_config()
        
        #print('Config :', self.response)
    #def place_order(self,id1, action, q_multiplier):
    #    id1 = int(self.data.loc[self.data['Description'] == item, ['ExchangeSegment','ExchangeInstrumentID']].iloc[0,1])
    #    order = self.xt.place_order(exchangeSegment='NSEFO', exchangeInstrumentID=id1, productType='NRML', orderType='MARKET',
    #                                orderSide=action, timeInForce=xt.VALIDITY_DAY, orderQuantity= self.quantity * q_multiplier, orderUniqueIdentifier="454845")
...
class XTS_order:
  def __init__(self, API_KEY, API_SECRET):
    ...
    self.xt = XTSConnect(API_KEY, API_SECRET, self.source)
    self.interactive = self.xt.interactive_login()
